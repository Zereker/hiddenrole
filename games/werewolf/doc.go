...
//
// 这件事由**编译器**保证，不靠自觉：本包在内核包之外，它能用的东西
// 使用者也能用（内核连 internal/ 都没有——enginetest 是公开的，理由见
// API.md）。想验证的话，看内核的 types.go——它的词汇表只有五个
...
// 快照、回放、审计这些能力才成立。
//
// 状态机认得的只有两条原语：改存活（NewSetAliveEffect）、写四种作用域的
// 变量、排一笔绕道的欠账。「狼刀」「放逐」「开枪」「守护」「解药」这些是
...
// 的能力由 Resolver 产出 NewDetourEffect，引擎会排队并自动流转
// 过去，它不需要认识任何具体角色。
//
// # 谁能知道什么
//
