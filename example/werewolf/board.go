...
func DefaultGameConfig() *GameConfig {
	return &GameConfig{
		...
		DefaultTimeout: hiddenrole.DefaultPhaseTimeout,
...
func StandardDayPhase() *PhaseConfig {
	return &PhaseConfig{
		...
		// 因此这里没有玩家技能步骤
		Steps: []PhaseStep{
			{Role: RoleSystem, Skill: SkillAnnounce},
		},
...
func StandardVotePhase() *PhaseConfig {
	return &PhaseConfig{
		...
		Steps: []PhaseStep{
			{Role: RoleSystem, Skill: SkillAnnounce},
			{Role: hiddenrole.RoleUnspecified, Skill: SkillVote, Required: true, Multiple: true},
		},
...
func DayHunterPhase() *PhaseConfig {
	return &PhaseConfig{
		...
		// 这里**不再**声明 ADVANCE_ROUND。
		//
		// 曾经要声明，是因为内核把「结算队列没排空时到期的回合边界」直接
		// 丢掉了：投票被猎人的死亡打断，VOTE 出口那次推进就没了，只能在
		// 延长赛的出口再补一次。那是规则替内核补差。
		//
		// 内核现在是**扣住、债清时补跑**：VOTE 出口那次推进会等到猎人开完
		// 枪、队列排空的那一次流转上落地，和清空回合状态同时发生。这里再
		// 声明一次，一个回合就会被数成两个。
	}
}
