...
第三套写下来只逼出**零个破坏性 API 变更**（[内核的稳定性政策](../../STABILITY.md)）。
...
- **可扩展** - 八个扩展点，内置角色与第三方走同一条路，没有特权；导出面由 golden 测试钉着（[稳定性政策](../../STABILITY.md)）
...
func main() {
	...
	must(g.SubmitSkillUse(&werewolf.SkillUse{
		PlayerID: "guard", Skill: werewolf.SkillProtect, Targets: []string{"seer"},
	}))
	next(g) // NIGHT_GUARD -> NIGHT_WOLF

	must(g.SubmitSkillUse(&werewolf.SkillUse{
		PlayerID: "w1", Skill: werewolf.SkillKill, Targets: []string{"v1"},
	}))
	must(g.SubmitSkillUse(&werewolf.SkillUse{
		PlayerID: "w2", Skill: werewolf.SkillKill, Targets: []string{"v1"},
	}))
	next(g) // NIGHT_WOLF -> NIGHT_WITCH

	// 女巫此刻可以看到刀口（解药还在手上）
	fmt.Printf("女巫看到的刀口: %s\n", werewolf.NightKillTarget(g))
	next(g) // NIGHT_WITCH -> NIGHT_SEER

	must(g.SubmitSkillUse(&werewolf.SkillUse{
		PlayerID: "seer", Skill: werewolf.SkillCheck, Targets: []string{"w1"},
	}))
...
func main() {
	...
	fmt.Printf("恢复后阶段=%v，女巫看到的刀口=%s\n", restored.PhaseInfo().Phase, witchSees(restored))
}
...
| 想知道 | 看哪份 |
|---|---|
| **内核有哪些 API、各承诺什么** | [API.md](../../API.md) · [STABILITY.md](../../STABILITY.md) |
| 内核**该**长成什么样、为什么这么抽象 | [DESIGN.md](../../DESIGN.md) |
...
选项装上去，与第三方注册自定义角色走同一批入口。覆盖率 94.7%（内核加三套规则包
一起统计），规则逐条对齐维基条目，每次测试跑 5000 局随机对局——三套规则包合计
（狼人杀 2000、任务制 1000、单夜制 2000）。

**破坏性变更此后走 `/v2`。** v1.5.0 曾被当作冻结点，之后 v1.6.0 又在 v1 线上删了
十三个导出名——那是 Go 的导入兼容规则不允许的。现在的政策是 v1 只加不删、破坏性
变更攒到下一个主版本一起走，理由与排队清单见
[STABILITY.md](../../STABILITY.md)。

**import 路径是 `github.com/Zereker/hiddenrole/example/werewolf`。** 内核与三套规则包
住在同一个 module 里，各自是独立的包。
...
Go 不让一个包碰另一个包的非导出标识符，所以本包能用的入口使用者也能用。想验证的话看 [`types.go`](../../types.go)：内核的
词汇表只有五个非空取值——`START`、`END`、`GOD`、`SKIP`、`ANNOUNCE`，外加
各类型的空零值。「女巫」「狼人」一个都没有，它们全在本包的
[`vocab.go`](vocab.go) 里。

**开一局只 import 本包就够。** 本包把内核的一小部分名字再导出了一遍
...
就是这么写的。内核的完整 API 见 [`API.md`](../../API.md)。
