...
// info 取玩家只读信息。
func (g *ruleGame) info(id string) hiddenrole.PlayerInfo {
	g.t.Helper()
...
func (g *ruleGame) setDead(ids ...string) {
	...
	for _, id := range ids {
		...
		g.e.Apply(hiddenrole.NewSetAliveEffect(id, false))
	}
}
...
func TestRule_R1_SeerChecksCamp(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.target, func(t *testing.T) {
			...
			if check.Data["camp"] != string(tc.wantCamp) {
				t.Errorf("查验 %s: 期望 camp=%v，实际 %v", tc.target, tc.wantCamp, check.Data["camp"])
			}
			if check.Data["isGood"] != strconv.FormatBool(tc.wantIsGood) {
...
func TestRule_R8_HunterMaySkipExplicitly(t *testing.T) {
	...
	if !hasSkip {
		t.Fatalf("前置不成立：hiddenrole.PhaseInfo 未宣告 SKIP 可用，实际 %v", advertised)
	}

	// 宣告了就必须能提交
	if err := g.use("h", SkillSkip, ""); err != nil {
		t.Fatalf("hiddenrole.PhaseInfo 宣告 SKIP 可用，SubmitSkillUse 却拒绝: %v", err)
	}
...
func TestRule_R10_SideWipeIgnoresEvilCategories(t *testing.T) {
	...
	e := MustNew(DefaultRules(), hiddenrole.WithRoleSetup(roleHiddenWolf,
	...
	e.Apply(hiddenrole.NewSetAliveEffect("s", false))
...
func TestRule_R10_RoleCategories(t *testing.T) {
	...
	x, _ := e2.PlayerInfo("x")
	if got := campOf(x); got != hiddenrole.CampUnspecified {
...
func TestRule_R11_DeadPlayerCannotVote(t *testing.T) {
	...
	if !hiddenrole.HasCode(err, hiddenrole.CodePlayerDead) {
...
func TestConvention_D3_SkillRejectedOutsideItsPhase(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if !hiddenrole.HasCode(err, hiddenrole.CodeSkillNotAllowed) {
