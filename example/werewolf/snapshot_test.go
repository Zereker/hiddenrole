...
func TestRestoreEngine_Rejects(t *testing.T) {
	...
	// 盘面版本读不懂不再是死路：快照里带着产生它的日志，重放一遍就能把
	// 盘面算回来。这正是 SnapshotVersion 升一位不再作废一代存档的原因。
	t.Run("盘面版本读不懂时靠日志重建", func(t *testing.T) {
		future := *valid
		future.Version = hiddenrole.SnapshotVersion + 1

		restored, err := Restore(nil, DefaultRules(), &future)
		if err != nil {
			t.Fatalf("应当靠日志重建，实际 %v", err)
		}

		// 重建这条路会丢一样东西，而且是写在 RestoreEngine 文档里的：
		// 已提交但尚未结算的技能从未变成效果，也就从未进过日志，只有盘面
		// 那半边存着。所以比较时把它摘掉单独断言。
		got := restored.Snapshot()
		if len(got.PendingUses) != 0 {
			t.Errorf("重建出的引擎不该带着待结算技能，实际 %v", got.PendingUses)
		}
		if len(valid.PendingUses) == 0 {
			t.Fatal("样本里本该有待结算技能，否则这一条断言不到东西")
		}

		lhs, rhs := *valid, *got
		lhs.PendingUses, rhs.PendingUses = nil, nil
		a, _ := json.Marshal(&lhs)
		b, _ := json.Marshal(&rhs)
		if string(a) != string(b) {
			t.Errorf("除待结算技能外，重建出的盘面应当一致:\n  期望 %s\n  实际 %s", a, b)
		}
	})

	t.Run("版本不兼容且没有日志可重建", func(t *testing.T) {
		bad := *valid
...
func TestRestoreEngine_RejectsInvalidPlayers(t *testing.T) {
	...
	snap := base()
	snap.Players[0].Role = RoleSystem
...
func TestPlayerVar_SurvivesSnapshotAndReplay(t *testing.T) {
	...
	// 效果流回放
	replayed, err := Replay(cfg, DefaultRules(), g.e.Log(), opts...)
	if err != nil {
		t.Fatalf("回放失败: %v", err)
	}
