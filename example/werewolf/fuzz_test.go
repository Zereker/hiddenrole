...
func playRandom(t *testing.T, seed int, rng *rand.Rand) []string {
	...
	// 回合边界可能欠着：声明推进的那个阶段结算时若还有死亡技能没排空，
	// 内核会扣住这次推进，等队列排空的那一次流转再落地。见不变量 H。
	boundaryOwed := false
	for step := 0; step < 400; step++ {
		...
		// 内核自己猜回合边界的旧设计。现在边界由板子声明（OnExit 里的
		// ActionAdvanceRound），于是能断言更强的东西：**回合数不该在别的
		// 地方偷偷跳**。
		//
		...
		declaredHere := declaresAction(cfg.Phases[lastPhase], hiddenrole.ActionAdvanceRound)
		if e.Status().Round > lastRound {
			// 允许两种来源：刚结算完的阶段自己声明了推进，或者之前欠下的
			// 那一次在这一步落地。除此以外回合数不该动。
			if !declaredHere && !boundaryOwed {
				t.Fatalf("seed=%d step=%d 回合数从 %d 跳到 %d，但刚结算完的 %v 没有声明 ADVANCE_ROUND，也没有欠着的边界",
					seed, step, lastRound, e.Status().Round, lastPhase)
			}
			if e.Status().Round != lastRound+1 {
				t.Fatalf("seed=%d step=%d 回合数一步跳了 %d：一次流转最多落一个边界",
					seed, step, e.Status().Round-lastRound)
			}
			boundaryOwed = false
		} else if declaredHere && !e.Status().Over {
			// 声明了却没动，只可能是被扣住了——那就该有债在排队。
			//
			// 对局结束是例外，且是刻意的：END 后面什么都没有，在进它的路上
			// 落一个边界，等于数了一个从未开始的回合，而且 END 不声明任何
			// 进入动作，回合状态会留在一个已经前进的计数器后面。
			if !hasPendingDetour(e) {
				t.Fatalf("seed=%d step=%d %v 声明了 ADVANCE_ROUND，回合数却没动，也没有待结算的死亡技能",
					seed, step, lastPhase)
			}
			boundaryOwed = true
		}
		...
		if e.Status().Round > lastRound {
			if rc := e.RoundContext(); rc != nil && len(rc.Vars) > 0 {
				t.Fatalf("seed=%d step=%d 进入第 %d 回合，上一回合的记录还在: %v（刚离开 %v）",
					seed, step, e.Status().Round, rc.Vars, lastPhase)
			}
			for _, id := range ids {
		...
		// 不变量 D：效果流回放必须与原引擎同步
		replayed, err := Replay(cfg, rules, e.Log(), opts...)
		if err != nil {
...
// hasPendingDetour 队列里还有没有待结算的死亡技能。
//
// 内核不导出这个判断（它是内部状态），但快照里带着——回合边界被扣住时，
// 靠它区分「扣住」与「丢了」。
func hasPendingDetour(e *Engine) bool {
	rc := e.RoundContext()
	return rc != nil && len(rc.Detours) > 0
}
