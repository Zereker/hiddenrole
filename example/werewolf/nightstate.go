...
// 「一局狼人杀的夜里会发生哪几种标记」被写进了内核。换一套规则——阿瓦隆
// 没有夜里的刀口，血染钟楼的标记有十几种——它们一个都用不上，而新规则
// 要表达自己的状态又只能回头去改内核的结构体。
//
// 现在它们只是键名。存储用内核的三种作用域，读写走通用原语，
...
func nightKillTarget(view GameView) string {
	return view.Var(hiddenrole.ScopeRound, RoundVarKillTarget)
}

// isProtected 该玩家今晚是否被守护。
func isProtected(view GameView, playerID string) bool {
	return view.Var(hiddenrole.ScopeRound.Of(playerID), PlayerRoundVarProtected) != ""
}

// isSaved 该玩家今晚是否被解药救下。
func isSaved(view GameView, playerID string) bool {
	return view.Var(hiddenrole.ScopeRound.Of(playerID), PlayerRoundVarSaved) != ""
}

// isPoisoned 该玩家今晚是否被毒。
func isPoisoned(view GameView, playerID string) bool {
	return view.Var(hiddenrole.ScopeRound.Of(playerID), PlayerRoundVarPoisoned) != ""
}
...
func lastProtected(view GameView, guardID string) string {
	round, err := strconv.Atoi(view.Var(hiddenrole.ScopeGame.Of(guardID), PlayerVarLastProtectedRound))
	if err != nil || round != view.Round()-1 {
		return ""
	}
	return view.Var(hiddenrole.ScopeGame.Of(guardID), PlayerVarLastProtectedTarget)
}

// markProtected 记下「本回合守卫守了谁」，供下回合判断连守。
func markProtected(view GameView, guardID, targetID string) []*Effect {
	return []*Effect{
		hiddenrole.NewSetVarEffect(hiddenrole.ScopeGame.Of(guardID), PlayerVarLastProtectedTarget, targetID),
		hiddenrole.NewSetVarEffect(hiddenrole.ScopeGame.Of(guardID), PlayerVarLastProtectedRound, strconv.Itoa(view.Round())),
	}
}
...
//
// 等价写法：e.Var(hiddenrole.ScopeRound, werewolf.RoundVarKillTarget)。
func NightKillTarget(e *Engine) string {
	return e.Var(hiddenrole.ScopeRound, RoundVarKillTarget)
}
