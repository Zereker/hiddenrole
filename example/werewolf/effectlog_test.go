...
func TestEffectLog_ReplayRebuildsGame(t *testing.T) {
	...
	replayed, err := Replay(nil, DefaultRules(), original.Log())
	if err != nil {
		t.Fatalf("回放失败: %v", err)
	}
...
func TestReplayEngine_Rejects(t *testing.T) {
	t.Run("含 nil 条目", func(t *testing.T) {
		if _, err := Replay(nil, DefaultRules(), logOf(nil)); err == nil {
			t.Error("应当拒绝 nil 条目")
		}
	})

	t.Run("开局效果缺少阶段", func(t *testing.T) {
		bad := logOf(hiddenrole.NewEffect(hiddenrole.EventGameStarted, "", ""))
	...
	t.Run("流转效果缺少阶段", func(t *testing.T) {
		bad := logOf(hiddenrole.NewEffect(hiddenrole.EventPhaseChanged, "", ""))
		if _, err := Replay(nil, DefaultRules(), bad); err == nil {
			t.Error("缺少阶段信息时应当报错")
		}
	})
	...
	t.Run("空日志得到未开局的引擎", func(t *testing.T) {
		e, err := Replay(nil, DefaultRules(), logOf())
		if err != nil {
			t.Fatal(err)
		}
...
func TestReplayEngine_MidRoundTriggerQueue(t *testing.T) {
	...
	replayed, err := Replay(nil, DefaultRules(), g.e.Log())
	if err != nil {
...
func TestApply_StaysReplayableAndRestorable(t *testing.T) {
	...
	// 一、回放
	replayed, err := Replay(DefaultGameConfig(), DefaultRules(), g.e.Log())
	if err != nil {
...
// logOf 把手搓的效果装成一份带版本号的日志。
//
// Replay 的参数从 []*Effect 换成了 *GameLog：日志现在是可持久化的记录，
// 自带格式版本号，而不再只是进程内的一串对象。构造坏样本因此要多包一层。
func logOf(effects ...*hiddenrole.Effect) *hiddenrole.GameLog {
	return &hiddenrole.GameLog{Version: hiddenrole.LogVersion, Effects: effects}
}
