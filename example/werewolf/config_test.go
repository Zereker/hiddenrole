...
func TestValidate_RequiresBothRoundDeclarations(t *testing.T) {
	for _, tc := range []struct {
		...
		{"没有阶段声明 ADVANCE_ROUND", func(c *GameConfig) {
			for _, pc := range c.Phases {
				pc.OnExit = withoutAction(pc.OnExit, hiddenrole.ActionAdvanceRound)
			}
		}, string(hiddenrole.ActionAdvanceRound)},
		{"没有阶段声明 CLEAR_ROUND_VARS", func(c *GameConfig) {
			for _, pc := range c.Phases {
				pc.OnEnter = withoutAction(pc.OnEnter, hiddenrole.ActionClearRoundVars)
			}
		}, string(hiddenrole.ActionClearRoundVars)},
	} {
		t.Run(tc.name, func(t *testing.T) {
			cfg := DefaultGameConfig()
			if err := cfg.Validate(); err != nil {
...
// withoutAction 去掉一项进出动作，用来构造「板子少了这一条」的反例。
//
// 这两条以前是 PhaseConfig 上的两个 bool，置 false 即可；现在它们是具名
// 动作列表里的一项，所以反例得把那一项摘掉。
func withoutAction(list []hiddenrole.PhaseAction, drop hiddenrole.PhaseAction) []hiddenrole.PhaseAction {
	out := make([]hiddenrole.PhaseAction, 0, len(list))
	for _, a := range list {
		if a != drop {
			out = append(out, a)
		}
...
// declaresAction 判断某阶段是否声明了某项进出动作。
func declaresAction(pc *hiddenrole.PhaseConfig, want hiddenrole.PhaseAction) bool {
	if pc == nil {
		return false
	}
	for _, list := range [][]hiddenrole.PhaseAction{pc.OnEnter, pc.OnExit} {
		for _, a := range list {
			if a == want {
				return true
			}
