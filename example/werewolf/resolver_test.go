...
func TestWitchResolver_Poison(t *testing.T) {
	...
	usePoisonEffects := roundMarks(effects)
...
func TestGuardResolver_Protect(t *testing.T) {
	...
	if got := len(roundMarks(effects)); got != 1 {
		t.Fatalf("expected one round mark, got %d in %v", got, effects)
	}
	if got := len(playerRecords(effects)); got != 2 {
...
func TestSeerResolver_CheckWolf(t *testing.T) {
	...
	if effects[0].Data["camp"] != string(CampEvil) {
...
func TestSeerResolver_CheckGood(t *testing.T) {
	...
	if effects[0].Data["camp"] != string(CampGood) {
...
// roundMarks / playerRecords 按**作用域**挑出写状态的效果。
//
// 内核原先有四个写状态的事件类型（SET_ROUND_VAR / SET_PLAYER_VAR / ...），
// 按类型过滤就等于按作用域过滤。四个后来合并成一个 SET_VAR，作用域改由
// 效果自己带着——所以过滤条件也得跟着搬到作用域上，光看类型会把「今晚被
// 守了」和「守卫上一回合守的是谁」混成一堆。
func roundMarks(effects []*Effect) []*Effect {
	return varEffectsIn(effects, func(ef *Effect, scope hiddenrole.VarScope) bool {
		return scope == hiddenrole.ScopeRound.Of(ef.TargetID) && ef.TargetID != ""
	})
}

func playerRecords(effects []*Effect) []*Effect {
	return varEffectsIn(effects, func(ef *Effect, scope hiddenrole.VarScope) bool {
		return scope == hiddenrole.ScopeGame.Of(ef.TargetID) && ef.TargetID != ""
	})
}

func varEffectsIn(effects []*Effect, want func(*Effect, hiddenrole.VarScope) bool) []*Effect {
	out := make([]*Effect, 0, len(effects))
	for _, ef := range effects {
		if scope, _, _, ok := ef.SetsVar(); ok && want(ef, scope) {
			out = append(out, ef)
		}
	}
	return out
