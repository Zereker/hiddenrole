...
func (r *VoteResolver) Resolve(uses []*SkillUse, view GameView) []*Effect {
	...
	if result.Tied || result.Winner == "" {
		effect := hiddenrole.NewEffect(EventVoteTied, "", "").
			WithData("result", "tied").
			WithData("votes", renderTally(result.Votes))
		effects = append(effects, effect)
		return effects
	}

	// 处决得票最多的玩家
	effect := hiddenrole.NewEffect(EventEliminate, "", result.Winner).
		WithData("votes", strconv.Itoa(result.MaxVote)).
		WithData("voters", strings.Join(sortedIDs(result.Voters[result.Winner]), ",")).
		WithData("allVotes", renderTally(result.Votes))
	effects = append(effects, effect, hiddenrole.NewSetAliveEffect(result.Winner, false))
...
func (r *GuardResolver) Resolve(uses []*SkillUse, view GameView) []*Effect {
	...
	firstUsePerPlayer(uses, func(use *SkillUse) {
		...
		protect := hiddenrole.NewEffect(EventProtect, use.PlayerID, use.Target())
		...
		default:
			...
			effects = append(effects,
				hiddenrole.NewSetVarEffect(hiddenrole.ScopeRound.Of(use.Target()), PlayerRoundVarProtected, VarPresent))
...
func (r *WolfResolver) Resolve(uses []*SkillUse, view GameView) []*Effect {
	...
	effects = append(effects,
		hiddenrole.NewSetVarEffect(hiddenrole.ScopeRound, RoundVarKillTarget, result.Winner))
...
func resolveAntidote(use *SkillUse, view GameView, rules Rules, killTarget string, blocked bool) ([]*Effect, bool) {
	save := hiddenrole.NewEffect(EventSave, use.PlayerID, use.Target())
	...
	default:
		...
		return []*Effect{
			save,
			hiddenrole.NewSetVarEffect(hiddenrole.ScopeGame.Of(use.PlayerID), VarWitchAntidote, ""),
			hiddenrole.NewSetVarEffect(hiddenrole.ScopeRound.Of(use.Target()), PlayerRoundVarSaved, VarPresent),
		}, true
	}
...
func resolvePoison(use *SkillUse, view GameView, blocked bool) ([]*Effect, bool) {
	poison := hiddenrole.NewEffect(EventPoison, use.PlayerID, use.Target())
	...
	default:
		...
		// 等于当场告诉全场谁被毒了。
		return []*Effect{
			hiddenrole.NewSetVarEffect(hiddenrole.ScopeGame.Of(use.PlayerID), VarWitchPoison, ""),
			hiddenrole.NewSetVarEffect(hiddenrole.ScopeRound.Of(use.Target()), PlayerRoundVarPoisoned, VarPresent),
		}, true
	}
...
func (r *SeerResolver) Resolve(uses []*SkillUse, view GameView) []*Effect {
	...
	firstUsePerPlayer(uses, func(use *SkillUse) {
		...
		check := hiddenrole.NewEffect(EventCheck, use.PlayerID, use.Target())
		// 只报阵营，不报具体角色
		if target, ok := view.Player(use.Target()); ok {
			check.
				WithData("camp", string(campOf(target))).
				WithData("isGood", strconv.FormatBool(campOf(target) == CampGood))
		}
...
func (r *NightResolveResolver) Resolve(uses []*SkillUse, view GameView) []*Effect {
	...
	if killTarget := nightKillTarget(view); killTarget != "" {
		...
		if dies {
			killEffect := hiddenrole.NewEffect(EventKill, "", killTarget)
			if reason != "" {
				killEffect.WithData("reason", reason)
			}
			effects = append(effects, killEffect, hiddenrole.NewSetAliveEffect(killTarget, false))
			...
			// 刀口未生效，清除击杀目标
			effects = append(effects,
				hiddenrole.NewSetVarEffect(hiddenrole.ScopeRound, RoundVarKillTarget, "").WithData("reason", reason))
		}
	}
	...
	for _, p := range view.AllPlayers() {
		...
		effects = append(effects,
			hiddenrole.NewEffect(EventPoison, "", p.ID),
...
func (r *HunterResolver) Resolve(uses []*SkillUse, view GameView) []*Effect {
	...
	firstUsePerPlayer(uses, func(use *SkillUse) {
		...
		case SkillSkip:
			// 猎人选择不开枪
			effects = append(effects,
				hiddenrole.NewEffect(EventSkip, use.PlayerID, ""))
		}
	})
...
func hunterTrigger(view GameView, playerID string, phase PhaseType) []*Effect {
	...
	return []*Effect{hiddenrole.NewDetourEffect(playerID, phase)}
}
...
func witchHas(view GameView, playerID string, kind potionKind) bool {
	...
	return view.Var(hiddenrole.ScopeGame.Of(playerID), key) != ""
}

// renderTally 把票数表渲染成一个字符串。
//
// 事件载荷现在只收字符串。理由见内核 Effect 的文档：效果日志是要写下来的
// 记录，而写不下来的记录不是记录。一张 map 该怎么渲染，只有产生它的一方
// 知道，内核不该去猜——顺手也把 map 迭代顺序不定这件事就地解决了：按 ID
// 排序，同一副盘面渲染出同一个字符串。
func renderTally(votes map[string]int) string {
	...
	for i, id := range ids {
		...
		b.WriteString(id)
		b.WriteByte(':')
		b.WriteString(strconv.Itoa(votes[id]))
	}
	return b.String()
}

// sortedIDs 复制并排序一份名单，同样是为了渲染稳定。
